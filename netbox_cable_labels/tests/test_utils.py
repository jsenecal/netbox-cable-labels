...
class RenderLabelTestCase(TestCase):
    ...
    @override_settings(
        PLUGINS_CONFIG={"netbox_cable_labels": {"label_template": "Cable-{{cable.pk}}-{{cable.status}}"}}
    )
    ...
    @override_settings(
        PLUGINS_CONFIG={
            "netbox_cable_labels": {"label_template": "{% if cable.length %}{{cable.length}}m{% else %}N/A{% endif %}"}
        }
    )
