...
class GenerateLabelsCommandTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        # Create rack
        cls.rack = Rack.objects.create(name="Test Rack", site=cls.site)
        ...
        # Create interfaces
        cls.interface_a = Interface.objects.create(device=cls.device_a, name="eth0", type="1000base-t")

        cls.interface_b = Interface.objects.create(device=cls.device_b, name="eth0", type="1000base-t")
    ...
    def test_generate_labels_for_cables_without_labels(self):
        ...
        cable1 = Cable(label="temp", a_terminations=[self.interface_a], b_terminations=[self.interface_b])
        cable1.save()
        Cable.objects.filter(pk=cable1.pk).update(label="")

        # Create another interface for second cable
        interface_a2 = Interface.objects.create(device=self.device_a, name="eth1", type="1000base-t")
        interface_b2 = Interface.objects.create(device=self.device_b, name="eth1", type="1000base-t")

        cable2 = Cable(label="temp", a_terminations=[interface_a2], b_terminations=[interface_b2])
        cable2.save()
    ...
    def test_generate_labels_skips_existing_labels(self):
        ...
        cable = Cable(label="Existing Label", a_terminations=[self.interface_a], b_terminations=[self.interface_b])
        cable.save()
    ...
    def test_generate_labels_handles_empty_queryset(self):
        ...
        # Create cable with a label
        cable = Cable(label="Has Label", a_terminations=[self.interface_a], b_terminations=[self.interface_b])
        cable.save()
